...
int depthwise(bm_handle_t &handle, param_t &param, const char *device_func_name) {
    ...
    std::string i_path = "./dep/case"+std::to_string(case_no)+"_i.dat";
    std::string k_path = "./dep/case"+std::to_string(case_no)+"_k.dat";
    std::string o_path = "./dep/case"+std::to_string(case_no)+"_o.dat";
...
//        kernel_host[i] = dist_value(rng);
    ...
    fread(kernel_host,sizeof(float),kernel_len,fp_k);
    // reference
